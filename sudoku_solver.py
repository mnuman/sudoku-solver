...
parser = argparse.ArgumentParser(description="Solve the sudoku.")
parser.add_argument(
    "--filename", type=str, help="comma separated input file", required=True
)
parser.add_argument("--mode", type=str, help="Special restrictions")


def filehelper(filename: str) -> list[list[Optional[int]]]:
    """Read the input sudoku from a comma separated text file, returning a list of lists
    where each element is either unknown (None) or the provided numeric value (1-9).
    """
    with open(filename, encoding="utf-8") as sudoku_file:
...
if __name__ == "__main__":
    ...
    lines: list[list[Optional[int]]] = filehelper(args.filename)
    sudoku: Sudoku = Sudoku(lines, args.mode)
    sudoku.setup_csp()
    sudoku.solve()
    print(f"--- Solved sudoku in {time.time() - start_time:5.3f} seconds ---")
    print()
    print(sudoku)
    print()
